...
const ProductCard = ({ item, query }: ProductCardProps) => {
  ...
  const images = useMemo(() => {
    ...
    // Handle database schema format where images are objects with url property
  ...
  // Initialize autoplay plugin with proper configuration
  const plugin = useRef(
    Autoplay({
      delay: 3000,
  ...
  useEffect(() => {
    if (!api || !plugin.current || images.length <= 1) return;
    ...
    try {
      ...
      console.warn('Autoplay control failed:', error);
    }
  }, [autoPlayActive, api, images.length]);
  ...
  return (
    <Card
      className={`relative w-full overflow-hidden group border border-muted bg-background/90 transition-all duration-300 rounded-xl p-0 ${item.isSold
    ...
    >
      <CardContent className="p-0">
        <div
          ...
          onMouseEnter={() => images.length > 1 && setAutoPlayActive(true)}
          onMouseLeave={() => setAutoPlayActive(false)}
        >
          <Carousel
            setApi={setApi}
            plugins={images.length > 1 ? [plugin.current] : undefined}
        ...
        <div className="p-4">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div className="flex-1 min-w-0">
              <h3 className={`font-semibold text-base leading-snug truncate transition-colors ${item.isSold ? 'text-muted-foreground' : 'group-hover:text-primary'
                }`}>
...
export const Products = () => {
  ...
  const allItems = useMemo(() => {
    ...
    // Handle both ItemsResponse and direct array formats
    ...
    // Handle response with data property (fallback)
    if ((itemsData as any).data && Array.isArray((itemsData as any).data)) {
  ...
  const items = useMemo(() => {
    ...
    return filters.hideSold
      ? allItems.filter((item: any) => item && !item.isSold)
      : allItems;
